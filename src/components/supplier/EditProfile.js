...
const EditProfile = ({ user: initialUser, onCancel }) => {
    const [user, setUser] = useState({ ...initialUser });
    const [submitted, setSubmitted] = useState(false);
    const [isUploading, setIsUploading] = useState(false);
    const [imageUrl, setImageUrl] = useState(initialUser.profile || null);
    ...
    const isSupplier = initialUser.userType === 'supplier';
    ...
    const uploadFile = async (e) => {
        ...
        data.append('upload_preset', REACT_APP_CLOUDINARY_UPLOAD_PRESET || 'ml_default');
        ...
        try {
            ...
            setUser(prev => ({ ...prev, profile: resData.secure_url }));
        } catch (err) {
        ...
        <form onSubmit={handleSubmit} className="space-y-10 animate-in fade-in duration-500">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 space-y-4">
                    ...
                    <div className={`relative border-2 border-dashed rounded-[2.5rem] h-[280px] flex flex-col items-center justify-center transition-all bg-slate-50/50 ${
                        ...
                        {isUploading ? (
                            ...
                            <div className="flex flex-col items-center gap-4 p-8 text-center">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <p className="text-[10px] font-bold text-slate-400 uppercase leading-tight">Company or personal logo</p>
                                </div>
                                <input 
                                    type="file" 
                                    accept="image/*" 
                ...
                <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <Input
                        ...
                        error={submitted && !user.names ? "Required" : null}
                    />
                    <Input
                        label="Email Address"
                        name="email"
                    ...
                    {isSupplier && (
                        <>
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Supplier Category</label>
                                <select
                                    name="supplierType"
            ...
            {registerFailure && (
                <div className="p-4 bg-rose-50 border border-rose-100 rounded-2xl flex items-center gap-3 text-rose-600 animate-in slide-in-from-top-2">
                    <AlertCircle size={20} />
                    <p className="text-xs font-bold">{registerFailure}</p>
                </div>
            )}
            ...
            <div className="flex items-center justify-end gap-3 pt-8 border-t border-slate-50">
                ...
                <Button 
                    ...
                    loading={registering || isUploading}
                    icon={Save}
                >
                    Save Changes
                </Button>
