			...
			<form onSubmit={handleSubmit} className="space-y-6">
				<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
					<Input
						...
						error={validated && !user.names ? 'Name is required' : null}
					/>
					<Input
						label="Email Address"
						name="email"
						...
						error={validated && !user.email ? 'Valid email is required' : null}
					/>
				</div>
				...
				<Input
					...
					error={validated && !user.subject ? 'Subject is required' : null}
				/>

				<div className="space-y-2">
					<label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 flex items-center gap-1.5">
						<MessageSquare size={12} className="text-primary" /> Detailed Message
					</label>
					<textarea
						name="message"
						id="message"
						...
						value={user.message}
						onChange={handleChange}
						className={`w-full bg-slate-50 border ${validated && !user.message ? 'border-red-200 focus:border-red-500' : 'border-slate-100 focus:border-primary'} rounded-2xl px-5 py-4 text-sm font-bold text-secondary outline-none focus:ring-4 focus:ring-primary/5 transition-all resize-none placeholder:text-slate-300`}
				...
				{contactFailed && (
					<div className="p-4 bg-red-50 border border-red-100 rounded-2xl flex items-center gap-3 text-red-600 animate-in shake duration-500">
						<AlertCircle size={20} />
						<p className="text-xs font-bold">{contactFailed}</p>
					</div>
				)}
			</form>
		</div>
