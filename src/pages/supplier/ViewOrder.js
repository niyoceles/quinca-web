...
const ViewOrder = () => {
  ...
  const order = useSelector(state => state.order.orderItem.oneorder);
  ...
  const calculateTotal = () => {
    return order.itemsArray.reduce((acc, item) => acc + (item.itemNumber * item.itemPrice), 0);
  };
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <Button 
              variant="ghost" 
              size="sm" 
              onClick={() => history.push('/account/supplier/orders')}
            ...
            <div>
              <Typography variant="h3">Order #{order.id.slice(-8).toUpperCase()}</Typography>
              <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">
                <Clock size={12} /> Received {dayjs(order.createdAt).fromNow()}
              </div>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            {order.isPaid && (
              <div className="px-6 py-2 bg-emerald-50 text-emerald-600 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-2 ring-1 ring-emerald-100">
                <ShieldCheck size={14} /> Fully Paid & Confirmed
              </div>
            )}
          </div>
        </div>
          ...
          <div className="lg:col-span-1 space-y-6">
            <Card className="border-none shadow-premium rounded-[2.5rem] bg-white p-8 space-y-8 overflow-hidden relative">
              ...
              <div className="pt-8 border-t border-slate-50">
                <Typography variant="h4" className="mb-6 flex items-center gap-2">
                  <Calendar size={18} className="text-primary" /> Delivery Schedule
                </Typography>
                <div className="space-y-4">
                  <div className="flex justify-between items-center bg-slate-50 p-4 rounded-2xl border border-slate-100/50">
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Preferred Date</span>
                    <span className="text-xs font-black text-secondary">{dayjs(order.needDate).format('MMM DD, YYYY')}</span>
                  </div>
                  <div className="flex justify-between items-center bg-slate-50 p-4 rounded-2xl border border-slate-100/50">
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Deadline</span>
                    <span className="text-xs font-black text-rose-500">{dayjs(order.deadline).format('MMM DD, YYYY')}</span>
                  </div>
                </div>
              </div>
            </Card>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/50 border-b border-slate-50">
                    ...
                    <th className="px-8 py-5 text-[10px] font-black text-slate-300 uppercase tracking-[0.2em] text-right">Unit Price</th>
                    <th className="px-8 py-5 text-[10px] font-black text-slate-300 uppercase tracking-[0.2em] text-right">Quantity</th>
                    <th className="px-8 py-5 text-[10px] font-black text-slate-300 uppercase tracking-[0.2em] text-right">Subtotal</th>
                  </tr>
                </thead>
                ...
                <tfoot>
                  <tr className="bg-slate-50/30">
                    <td colSpan={3} className="px-8 py-8 text-right">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Grand Total Balance</p>
                    </td>
